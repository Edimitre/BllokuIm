...
class ExpenseService @Inject constructor(private val expenseDao: ExpenseDao) {
    ...
    var sizeOfExpensesThisYear =
        expenseDao.getSizeOfExpenseListByYear(TimeUtils().getCurrentYear()).asLiveData()

    var valueOfExpensesThisYear =
        expenseDao.getValueOfExpenseListByYear(TimeUtils().getCurrentYear()).asLiveData()
    ...
    var valueOfThisMonthExpenses = expenseDao.getValueOfExpenseListByYearMonth(
        TimeUtils().getCurrentYear(),
        TimeUtils().getCurrentMonth()
    ).asLiveData()

    var sizeOfThisMonthExpenses = expenseDao.getSizeOfExpenseListByYearMonth(
        TimeUtils().getCurrentYear(),
        TimeUtils().getCurrentMonth()
    ).asLiveData()
    ...
    var valueOfTodayExpenses = expenseDao.getValueOfExpenseListByYearMonthDate(
        TimeUtils().getCurrentYear(),
        TimeUtils().getCurrentMonth(), TimeUtils().getCurrentDate()
    ).asLiveData()

    var sizeOfTodayExpenses = expenseDao.getSizeOfExpenseListByYearMonthDate(
        TimeUtils().getCurrentYear(),
        TimeUtils().getCurrentMonth(), TimeUtils().getCurrentDate()
    ).asLiveData()


    var biggestExpenseThisMonth = expenseDao.getBiggestExpenseByYearMonth(
        TimeUtils().getCurrentYear(),
        TimeUtils().getCurrentMonth()
    ).asLiveData()

    var biggestExpenseThisYear =
        expenseDao.getBiggestExpenseByYear(TimeUtils().getCurrentYear()).asLiveData()
